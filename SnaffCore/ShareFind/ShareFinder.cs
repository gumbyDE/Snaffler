    ...
    public class ShareFinder
    {
        ...
        internal void GetComputerShares(string computer)
        {
            ...
            foreach (HostShareInfo hostShareInfo in hostShareInfos)
            {
                ...
                {
                    bool matched = false;
                    // prepare the share name for comparison
                    // we want it uppercase and want any appended dollar letters gone
                    string shareNamePrepared = hostShareInfo.shi1_netname
                        .ToUpper()
                        .TrimEnd('$');
